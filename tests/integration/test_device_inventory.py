...
def test_collect_configs_from_topology(monkeypatch, tmp_path):
    ...
    sample = json.load(open(MOCKS_DIR / "ssh_output_1.json"))
    class MockSSHClient:
    ...
    topology = "testtopo_collect"
    client.post(f"/topology/inventory/add?inventory={topology}", json=device)
    # Save configs
...
def test_save_configs_returns_per_host_message(monkeypatch, tmp_path):
    ...
    for i in range(10, 13):
        ...
        for result in data["results"]:
            if result["host"] == host:
                assert f"Configuration for host {host} saved to {output_folder}/{host}_config.txt" in result["message"]
                found = True
