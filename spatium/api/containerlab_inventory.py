...
async def deploy_containerlab_topology(
    body: Dict[str, Any] = Body(..., description="Topology configuration with links"),
    ...
    try:
        ...
        logger.info(f"Building topology '{topology_name}' with {len(nodes)} nodes from inventory '{inventory}'")
        
        # Build links in ContainerLab format
        clab_links = []
        for i, link in enumerate(links):
            try:
                # Accept both {a, b} and {endpoints: [...]} for backward compatibility
        ...
        response = requests.post(
            api_endpoint,
            json=topology,
            timeout=settings.CONTAINERLAB_TIMEOUT
        )
