...
from spatium.api.dependencies import get_inventory_service, get_device_config_service
...
async def get_device_configs(
    ...
    try:
        ...
        raise create_http_exception(500, f"Failed to fetch device configurations: {e}")
...
async def save_device_configs(
    ...
    try:
        ...
        raise create_http_exception(500, f"Failed to save device configurations: {e}")
...
async def get_single_device_config(
    ...
    try:
        ...
        raise create_http_exception(500, f"Failed to fetch device configuration: {e}")


@router.post("/test-connectivity")
async def test_device_connectivity(
    devices: List[DeviceConfigRequest] = Body(..., description="List of devices to test"),
    config_service: DeviceConfigService = Depends(get_device_config_service),
):
    """Test connectivity to devices using their specified connection method."""
    try:
            ...
            if method == "rest":
                ...
                try:
                    rest_client = config_service.rest_client_factory(
                        base_url=f"http://{device.host}:{device.port or 80}",
        ...
        return {
            "success": True,
            "message": f"Tested connectivity to {len(devices)} device(s). {successful_connections} successful.",
            "results": connectivity_results
        }
        
    except Exception as e:
        logger.error(f"Error testing device connectivity: {e}")
        raise create_http_exception(500, f"Failed to test device connectivity: {e}")
