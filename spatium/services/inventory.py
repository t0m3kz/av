...
from spatium.api.exceptions import InventoryError, DeviceNotFoundError
...
class InventoryService:
    ...
    def remove_devices(
        ...
        self._inventories[inventory_name] = [
            ...
            if device.host not in device_hosts
        ]
        
        removed_count = original_count - len(self._inventories[inventory_name])
        removed_hosts = list(device_hosts.intersection(
            {device.host for device in inventory}
        ))
    ...
    def get_inventory_stats(self, inventory_name: str = "default") -> Dict[str, int]:
